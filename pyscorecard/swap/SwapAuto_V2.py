...
def Swap(df,y,P1,P2,P1_name,P2_name,CutNumP1,CutNumP2,filename,outputh,GroupNums1=10,GroupNums2=10):
    ...
    try:
        ...
        flag_list =list(df['flag'].unique())
        book = xlwt.Workbook(encoding='utf-8')
    
        for flag in flag_list:
            
            df_tmp =df.iloc[np.where(df.flag==flag)[0],:]
        
            ks_value,bad_percent,good_percent=cal_ks(np.array(df_tmp[P1]),np.array(df_tmp[y]),section_num=10)
            false_positive_rate, recall, thresholds = roc_curve(df_tmp[y], df_tmp[P1])
            roc_auc=auc(false_positive_rate,recall)
            P1_ks =np.max(ks_value)
            P1_auc =roc_auc
            
            ks_value,bad_percent,good_percent=cal_ks(np.array(df_tmp[P2]),np.array(df_tmp[y]),section_num=10)
            false_positive_rate, recall, thresholds = roc_curve(df_tmp[y], df_tmp[P2])
            roc_auc=auc(false_positive_rate,recall)
            P2_ks =np.max(ks_value)
            P2_auc =roc_auc
        
            P1_rank,P2_rank,df_swap_len,df_swap_sum,df_swap_len_rate,df_swap_sum_rate =SwapData(df_tmp, y,P1,P2,GroupNums1,GroupNums2)
            ...
            sheet = book.add_sheet(str(flag)+'交叉分析')
            
            ##auc ks rank写入
            corr =np.corrcoef(df_tmp[P1],df_tmp[P2])[0,1]
            AucKsExcelFormat(sheet,1,0,"模型效果",P1_name,P2_name,corr.round(4))
            sheet.write(2,1,P1_auc.round(4),style)
            sheet.write(2,2,P2_auc.round(4),style)
            sheet.write(3,1,P1_ks.round(4),style)
            sheet.write(3,2,P2_ks.round(4),style)
            
            ## Rank写入
            RankExcelFormat(sheet,1,4,"评分排序",P1_name,P2_name,GroupNums=10)
            WriteData2(Rank_bad,sheet,2,5,GroupNums1,GroupNums2)
            
            ## Swap写入
            SwapExcelFormat(sheet,18,0,"逾期率",P1_name,P2_name,GroupNums1,GroupNums2)
            WriteData3(df_swap_sum_rate,sheet,20,1)
            
            SwapExcelFormat(sheet,18,GroupNums2+3,"人数占比",P1_name,P2_name,GroupNums1,GroupNums2)
            WriteData3(df_swap_len_rate,sheet,20,GroupNums2+4)
            
            SwapExcelFormat(sheet,30+GroupNums1,0,"总体人数分布",P1_name,P2_name,GroupNums1,GroupNums2)
            WriteData(df_swap_len,sheet,32+GroupNums1,1)
            
            SwapExcelFormat(sheet,30+GroupNums1,GroupNums2+3,"逾期人数分布",P1_name,P2_name,GroupNums1,GroupNums2)
            WriteData(df_swap_sum,sheet,32+GroupNums1,GroupNums2+4)
        
            ## swapinout data 写入
            df_tot_bad,df_tot_bad_rate =SwapInOutData(df_swap_len,df_swap_sum,CutNumP1,CutNumP2,GroupNums1,GroupNums2)
            SwapInOutExcelFormat(sheet,23+GroupNums1,0,P1_name,P2_name,CutNumP1,CutNumP2)
            WriteData(df_tot_bad,sheet,25+GroupNums1,1)
            WriteData3(df_tot_bad_rate,sheet,25+GroupNums1,4)
    except:
